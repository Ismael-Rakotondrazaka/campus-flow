#!/usr/bin/env tsx
/**
 * Creates the three S3 buckets required by campus-flow on a SeaweedFS (or
 * any S3-compatible) backend and applies a public-read policy to the two
 * public buckets.
 *
...
const BUCKETS: { name: string; public: boolean }[] = [
  { name: 'campus-flow-user-profiles', public: true },
  { name: 'campus-flow-announcement-illustrations', public: true },
  { name: 'campus-flow-housing-application-documents', public: false },
  { name: 'campus-flow-renewal-documents', public: false },
];
...
runMain(
  defineCommand({
    meta: {
      description:
        'Create campus-flow S3 buckets and apply public-read policies where needed',
