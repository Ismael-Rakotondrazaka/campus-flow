...
import { authClaimsQuery } from '~/features/auth/auth.query';
...
const { data: claims, isPending: isClaimsPending } =
  useQuery(authClaimsQuery());
const { data: user, isPending: isUserPending } = useQuery(() => ({
  ...userByIdQuery({ id: authUser.value?.sub ?? '' }),
  enabled: Boolean(authUser.value?.sub),
}));
</script>
